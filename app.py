...
def order():
    ...
    if request.method == "POST":
        # 1. Pobierz zaznaczone pozycje z formularza
        pozycje = []
        ...
        # 4. Ustal opcję dietetyczną z pierwszej pozycji
        cursor.execute("""
            SELECT m.opcja_dietetyczna_id
            FROM pozycje_menu pm
            JOIN menu m ON pm.menu_id = m.id
            WHERE pm.id = %s
            LIMIT 1
        ...
        if not dieta_info:
            flash('Nie znaleziono diety dla wybranej pozycji.')
            return redirect(url_for('order'))

        opcja_dietetyczna_id = dieta_info['opcja_dietetyczna_id']

        # 5. Wstaw zamówienie do bazy z dietą
        import random
        kod_unikalny = random.randint(1000, 9999)

        cursor.execute("""
            INSERT INTO zamowienie (
                uzytkownik_id, dostawca_id, data_dostawy, 
        ...
        """, (
            user_id, dostawca['id'],
            user['adres'], user['miejscowosc'], user['kod_pocztowy'], user['nr_domu'],
            1, kod_unikalny, True, opcja_dietetyczna_id
        ))

        zamowienie_id = cursor.lastrowid

        # 6. Dodaj pozycje zamówienia
